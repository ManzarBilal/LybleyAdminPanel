...
function MySparePartEdit(props) {
    ...
    const editProduct = async () => {
        ...
        let obj={productId:pdId?._id,partName:sparePart?.partName,description:sparePart?.description,
            MRP:sparePart?.MRP,bestPrice:sparePart?.bestPrice,faultType:sparePart?.faultType,partNo:sparePart?.partNo,skuNo:sparePart?.skuNo,length:sparePart?.length,breadth:sparePart?.breadth,height:sparePart?.height,weight:sparePart?.weight};
        try {
            setLoading(true);
            let response = await httpCommon.patch(`/updateCompactibleSparePart/${id}`,obj);
            let { data } = response;
            setLoading(false);
            ToastMessage(data);
            ...
            <div className="row g-3">
                ...
                <div className="col-xl-12 col-lg-12">
                    <div className="card mb-3">
                        <MyEditBasicInformation user={user} onDelete={handleFaultDelete} onSubmit={handleFault} faultType={faults} sparePart={sparePart} onChange={handleChange} />
                    </div>
